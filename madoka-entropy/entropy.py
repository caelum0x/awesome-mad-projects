...
karmic cost dumped into the global reservoir. That asymmetry is what keeps
the total non-decreasing. See README.md for the honest framing of why this
is a caricature of thermodynamics rather than a literal derivation.
"""
...
def wish_deltas(local_order: float, karmic_multiplier: float) -> tuple:
    ...
    Net total change = (karmic_multiplier - 1) * local_order > 0, which is the
    per-wish guarantee behind the global invariant.

    Returns (d_global, d_local). Raises ValueError on invalid parameters so the
    invariant can never be silently violated by bad input.
    """
    if local_order <= 0:
        raise ValueError(f"local_order must be > 0, got {local_order}")
    if karmic_multiplier <= 1.0:
        raise ValueError(
            f"karmic_multiplier must be > 1 to preserve the 2nd law, "
