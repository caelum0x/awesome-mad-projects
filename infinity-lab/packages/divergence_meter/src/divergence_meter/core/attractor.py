...
value into its field, and report the distance to the nearest field boundary (how
close the current line is to slipping into a neighbouring cluster).
...
# Half-open [low, high) bins spanning the full [0, 2) divergence range. The
# canonical split is Alpha < 1.0 and Beta >= 1.0; we subdivide each half for a
# little more texture.
...
def classify(value: float) -> FieldClassification:
    ...
    Args:
        value: A divergence value, expected in ``[0, 2)``.
