...
Each digit is drawn in a 3-wide, 5-tall cell using a classic seven-segment font.
Tubes are boxed to evoke the orange glow of the Divergence Meter without
requiring any colour support -- pure ASCII, works in any terminal.

This is an adapter (presentation layer): it imports only the standard library and
...
def render(display: str) -> str:
    ...
    Raises:
        ValueError: If ``display`` is empty.
    """
    if not display:
