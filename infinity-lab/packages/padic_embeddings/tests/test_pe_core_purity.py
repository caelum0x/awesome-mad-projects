"""Guard: padic_embeddings.core must stay pure (stdlib + commons.core only).

Statically scans every module under ``padic_embeddings/core`` and asserts none of
them import an adapter layer (render / cli / viz) or hard-import a scientific optional
dependency (numpy / matplotlib) or ``commons.adapters``. It also asserts, dynamically
...
import padic_embeddings.core as core_pkg
...
def test_importing_core_does_not_load_numpy() -> None:
    ...
    # this session has already pulled numpy into this process's sys.modules.
    script = (
        "import sys; import padic_embeddings.core;"
    ...
    assert result.returncode == 0, (
        "importing padic_embeddings.core pulled numpy/matplotlib into "
