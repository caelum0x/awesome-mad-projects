...
contracted girl; purity ``0.0`` is total corruption -> witch transformation.
Every act of magic (every wish/spell that imposes local order) muddies the gem
in proportion to how much order was forced onto the world, because fighting
local entropy is what accrues karmic corruption.
...
class MagicalGirl:
    ...
    local_entropy: float   # subsystem entropy (lower = more order imposed)
    is_witch: bool = False
    ...
    def cast(self, local_order: float, decay_per_order: float) -> "MagicalGirl":
        ...
        her purity decays by ``decay_per_order * local_order``. Purity is
        clamped at 0 so downstream code can detect the witch threshold crossing.
        """
        if local_order < 0:
            raise ValueError(f"local_order must be >= 0, got {local_order}")
        if decay_per_order < 0:
            raise ValueError(f"decay_per_order must be >= 0, got {decay_per_order}")
...
def make_girls(names: Iterable[str], base_local_entropy: float) -> Tuple[MagicalGirl, ...]:
