"""Tests for the OPTIONAL matplotlib PNG export (central_finite_curve.adapters.viz).
...
is needed. Each test renders a SMALL-config projection to ``tmp_path`` and asserts the
...
from central_finite_curve.adapters import cli, viz
...
def test_save_projection_png_writes_valid_png(tmp_path) -> None:
    target = str(tmp_path / "projection.png")
    out = viz.save_projection_png(target, config=_SMALL)
    assert out == target
    _assert_valid_png(target)


def test_cli_png_option_writes_projection(tmp_path) -> None:
    outdir = tmp_path / "pngs"
    out = cli.run_cli(
        ["project", "--universes", "600", "--walk-steps", "200", "--png", str(outdir)]
    )
    assert "PNG export written" in out
    target = str(outdir / "central_finite_curve_projection.png")
...
def test_cli_png_creates_missing_outdir(tmp_path) -> None:
    ...
    cli.run_cli(
        ["all", "--universes", "600", "--walk-steps", "200", "--png", str(outdir)]
    )
    _assert_valid_png(str(outdir / "central_finite_curve_projection.png"))
