...
metric on the raw angle values gets this wrong.
"""
from __future__ import annotations

import math

from latent import LatentPoint, CompactifiedLatentSpace, TWO_PI
...
def naive_distance(p: LatentPoint, q: LatentPoint) -> float:
    ...
    This is the "naive" observer: it ignores the periodicity of the compact
    dimensions and compares raw angle values as if they lived on the real line.
    """
    total = 0.0
...
def naive_angular_distance(p: LatentPoint, q: LatentPoint) -> float:
    ...
    Isolates the topology effect: compares raw angle values on the real line,
    so points straddling the 0 / 2*pi seam look artificially far apart.
    """
    total = 0.0
    for a, b in zip(p.angles, q.angles):
        total += (a - b) ** 2
...
def toroidal_distance(
    ...
    Arc length on circle j is radius_j * |shortest_angular_delta|.
    """
    total = 0.0
    for a, b in zip(p.extended, q.extended):
        total += (a - b) ** 2
