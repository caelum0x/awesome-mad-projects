"""Nearest-neighbor + tiny clustering under a pluggable distance function.

We use a simple, dependency-free connected-components clustering: connect any
two points whose distance is below a threshold, then take connected components.
This is enough to show that the *choice of metric* (naive vs topology-aware)
changes which points are grouped together.
"""
from __future__ import annotations

from typing import Callable, List, Sequence

from latent import LatentPoint
...
def nearest_neighbor(
    ...
    """Return index of the closest point under `dist` (excluding exact self)."""
    best_idx = -1
    best_d = float("inf")
...
def cluster(
    ...
    them. Immutable style: builds fresh lists, never mutates inputs.
    """
    n = len(points)
    parent = list(range(n))
    ...
    # Relabel roots to compact 0..K-1 labels.
    roots = {}
    labels: List[int] = []
    for i in range(n):
        r = find(i)
