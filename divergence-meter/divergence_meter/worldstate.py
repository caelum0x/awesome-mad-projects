"""Gather a deterministic snapshot of "world state".
...
Design notes:
    * Snapshot is a frozen dataclass -> immutable, no shared-state mutation.
    * We never trust external data blindly; every branch validates its input
      and raises a clear WorldStateError on failure.
"""
...
def _directory_payload(path: str) -> bytes:
    ...
    makes the result independent of filesystem iteration order, so the same
    tree always yields the same bytes.
    """
    entries: list[str] = []
    for root, dirs, files in os.walk(path):
        # Sort in-place copies so traversal is deterministic (no mutation of
        # the original lists returned by os.walk beyond what walk expects).
        dirs.sort()
...
def _canonical_json(text: str) -> bytes | None:
