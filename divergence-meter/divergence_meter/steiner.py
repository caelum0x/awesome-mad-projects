...
"Reading Steiner" is Okabe Rintaro's ability to retain memories across
worldline shifts. Here it is a tiny JSON-backed store: you ``save`` the current
divergence reading under a name, and later ``jump`` back to it to see the delta
between where you are now and the line you recorded.

The store is a plain JSON object mapping name -> record. All writes are done by
rewriting a new dict (never mutating the loaded one in place) and then atomically
replacing the file.
"""
...
from .divergence import DivergenceReading

# Default store lives inside the project directory so the tool is fully
# self-contained and never writes elsewhere on the system.
_PROJECT_ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
DEFAULT_STORE_PATH = os.path.join(_PROJECT_ROOT, "worldlines_store.json")
...
def get_line(name: str, *, store_path: str = DEFAULT_STORE_PATH) -> WorldlineRecord:
    ...
    if name not in data:
        ...
        raise SteinerError(
            f"No worldline named '{name}'. Saved lines: {available}."
        )
    entry = data[name]
    try:
